...
class KCBJSONParser(object):
    ...
    def parse_hits(self):
        ...
        logger.debug('KCBJSONParser: collected {} total hit entries'.format(len(self.collected_hits)))
    ...
    def _parse(self, record):
        ...
        for i, result in enumerate(kcb['results']):
            ...
            sig_hits = [(entry[0]['accession'], entry[0]['description']) for entry in result['ranking']]
...
class KCBTextParser(object):
    ...
    def __init__(self, filename):
        if not os.path.exists(filename):
            raise Exception('KCBTextParser failed to find file "{}"'.format(filename))
        ...
        with open(filename, 'r') as f:
            ...
            for i, detail in enumerate(details):
                ...
                if len(all_mibig_genes) == 0:
                    logger.warning('KCBTextParser failed to extract any MiBIG genes from file {}, BGC ID {}'.format(filename, current_bgc_id))
