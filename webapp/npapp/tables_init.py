...
def setup_data_tables(nh):
    ...
    nh.nplinker.scoring.metcalf.cutoff = 4
    ...
    # construct pandas dataframes for each object class
    bgc_data = dict(bgc_pk=[NA], name=[NA], product_type=[NA], score=[NA])
    gcf_data = dict(gcf_pk=[NA], gcf_id=[NA], product_type=[NA], nbgcs=[NA], score=[NA])
    spec_data = dict(spec_pk=[NA], spectrum_id=[NA], family=[NA], score=[NA])
    molfam_data = dict(molfam_pk=[NA], family_id=[NA], nspectra=[NA], score=[NA])

    # TODO scoring needs figured out
    ...
    for gcf in nh.nplinker.gcfs:
        ...
        gcf_data['nbgcs'].append(len(gcf.bgcs))
    ...
    for molfam in nh.nplinker.molfams:
        ...
        molfam_data['nspectra'].append(len(molfam.spectra))
    ...
    gcf_cols = [TableColumn(field='gcf_pk', title='ID'), TableColumn(field='gcf_id', title='GCF ID'), TableColumn(field='product_type', title='Product type'), TableColumn(field='nbgcs', title='#bgcs'), TableColumn(field='score', title='Score')]
    ...
    molfam_cols = [TableColumn(field='molfam_pk', title='ID'), TableColumn(field='family_id', title='MolFam ID'), TableColumn(field='nspectra', title='#spectra'), TableColumn(field='score', title='Score')]
    ...
    bgc_dt = DataTable(source=bgc_ds, columns=bgc_cols, width=300, name='table_bgcs', visible=False)
    gcf_dt = DataTable(source=gcf_ds, columns=gcf_cols, width=300, name='table_gcfs', visible=False)
    spec_dt = DataTable(source=spec_ds, columns=spec_cols, width=300, name='table_spectra', visible=False)
    molfam_dt = DataTable(source=molfam_ds, columns=molfam_cols, width=300, name='table_molfams', visible=False)
    ...
    bgc_gcf = create_links(bgc_df, gcf_df, 'bgc_pk', 'gcf_pk', bgc_to_gcf_indices)
    ...
    mf_spectra = create_links(molfam_df, spec_df, 'molfam_pk', 'spec_pk', molfam_to_spec_indices)

    # 3. links between GCF<==>Spectrum objects based on metcalf scores, via BGCs
    spec_to_bgc_indices = []
    for spectrum in spec_links:
        # lookup the GCF that this spectrum has a link to 
        linked_gcfs = nh.nplinker.links_for_obj(spectrum, nh.nplinker.scoring.metcalf)
        for gcf, score in linked_gcfs:
            #print('spec {} <==> gcf {}'.format(spectrum.id, gcf.id))
            for bgc in gcf.bgcs:
                spec_to_bgc_indices.append((spectrum.id + 1, bgc.id + 1))
    spectra_bgc = create_links(spec_df, bgc_df, 'spec_pk', 'bgc_pk', spec_to_bgc_indices)
    ...
    table_widgets = [bgc_dt, gcf_dt, spec_dt, molfam_dt]
    load_tables = Button(label='Load table data', name='load_tables')
    load_tables.callback = CustomJS(args=dict(tableInfo=table_info, tableWidgets=table_widgets), code="""
