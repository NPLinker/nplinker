    ...
    for l in csv.reader(f):
        ...
        if mibig_inchi_first_block in mibig_inchi_keys:
            mibig_inchi_keys[mibig_inchi_first_block].append(
                ((mibig_id, compound_name, pubchem_id), mibig_inchi_key, smiles)
            )  # .split('_')[0])
        else:
...
for gnps_spectrum in mgf.read(
    "/home/grimur/iokr/data/mibig/gnps-mibig/gnps_with_structure.mgf", use_index=False
):
