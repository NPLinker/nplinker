...
def config():
    return {
        "dataset": {
            ...
            "platform_id": "",
            "overrides": {"strain_mappings_file": str(DATA_DIR / STRAIN_MAPPINGS_FILENAME)},
        }
    }
...
def config_with_new_gnps_extractor():
    GNPSExtractor(
        DATA_DIR / "ProteoSAFe-METABOLOMICS-SNETS-c22f44b1-download_clustered_spectra.zip",
        DATA_DIR / "extracted",
    )
    yield {
        "dataset": {
...
def test_has_metabolomics_paths_new_gnps(config_with_new_gnps_extractor):
    ...
    assert sut.edges_file == str(
        config_with_new_gnps_extractor["dataset"]["root"] / "molecular_families.tsv"
    )
    assert sut.nodes_file == str(
        config_with_new_gnps_extractor["dataset"]["root"] / "file_mappings.tsv"
    )
    assert sut.annotations_dir == str(config_with_new_gnps_extractor["dataset"]["root"])
    assert sut.annotations_config_file == str(
        config_with_new_gnps_extractor["dataset"]["root"] / "annotations.tsv"
    )
